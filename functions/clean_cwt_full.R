clean_cwt_full <- function(data_cwt_full){
  ...
  data_cwt_full <- rename(data_cwt_full, replace = c("predictionLevel" = "Trial_type"))
  data_cwt_full$Trial_type <- revalue(data_cwt_full$Trial_type, c("pred"="Predictive", "nonpred"="Non-predictive", "antipred"="Anti-Predictive"))
  ...
  data_cwt_full$Trial_type <- as.factor(data_cwt_full$Trial_type)
  data_cwt_full$Ambiguity <- as.factor(data_cwt_full$Ambiguity)
  data_cwt_full$record_id <- as.factor(data_cwt_full$record_id)
  data_cwt_full$Trial_type<- factor(data_cwt_full$Trial_type, levels=rev(levels(data_cwt_full$Trial_type)))
